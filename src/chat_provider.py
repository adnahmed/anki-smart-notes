...
class ChatProvider:
    async def async_get_chat_response(
        ...
        # Otherwise, return an error - server backend no longer available
        raise RuntimeError(
            f"Smart Notes server backend is not available. Please use Ollama (local) or configure your OpenAI API key for {provider}."
        )
